...
- **무엇**: 2026 국토교통 데이터 활용 경진대회 출품작 (제품/서비스 트랙). 코드네임 `BlindZone` — 사고 위험 × 응급 사각지대 데이터 융합으로 잠재 위험지대 발굴하는 풀스택 웹앱 (Next.js + FastAPI).
- **목적/맥락**: 국토교통부 주최 경진대회 입상 목표. 총상금 3,400만원 / 10건 시상. 가점 신청 항목 (심사위원 판단): AI 학습·분석(각 5점), 데이터융합·가명정보·안심구역(각 5점). BlindZone 신청 예정: AI 학습+분석+데이터융합 = 15점.
- **마감/일정**: 2026-05-29 접수 마감. 1차 심사 5~6월 (2배수 선발), 멘토링·2차 심사 6~7월, 최종발표·시상 7월.
- **팀/혼자**: 혼자 (1인 출품)
- **기술 스택**: 풀스택 — Next.js 14 (App Router) + TypeScript + Tailwind + MapLibre GL + deck.gl + Recharts (프론트) / FastAPI + XGBoost + SHAP + GeoPandas (백엔드) / Vercel + Railway (배포). 디렉토리는 `backend/` + `frontend/` 분리.
- **기타**: 한태영은 교통시스템공학과 재학. 직감 좋고 신기술 빨리 흡수. 가용시간 많음. 파주시 AI 공모전 등 경진대회 경험 다수. 원본 대회 문서: `대회정보.txt`, `2026경진대회_기획서(최종).pdf`.

**절대 원칙**: 없는 것을 지어내지 않는다. 데이터·통계·작동결과·가점 모두 검증된 사실만. 모호하면 명시 마킹 (`[확인 필요]` 등). 자세히는 memory 참고.
